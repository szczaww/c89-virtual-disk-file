...
#define FIRSTFIT_MODE 0
#define WORSTFIT_MODE 1
...
PRIVATE int ALLOC_MODE = FIRSTFIT_MODE;
...
PUBLIC int do_holemap(void) {
  ...
  while (hp != NIL_HOLE && hp->h_base < swap_base && buffer_end < 1022) {
    buffer[buffer_end++] = hp->h_len;
    buffer[buffer_end++] = hp->h_base;
...
PUBLIC int do_worstfit(void) {
  if (mm_in.m1_i1 == FIRSTFIT_MODE || mm_in.m1_i1 == WORSTFIT_MODE ) {
    ALLOC_MODE = mm_in.m1_i1;
    return 0;
  }
  return -1;
}
 ...
 */
  ...
  register struct hole *max_hp, *max_prev_ptr;
  phys_clicks old_base;


  if (ALLOC_MODE == FIRSTFIT_MODE) {
    do {
    hp = hole_head;
  ...
  else if (ALLOC_MODE == WORSTFIT_MODE) {
    do {
      hp = hole_head;
      max_hp = hole_head;
      ...
      while (hp != NIL_HOLE && hp->h_base < swap_base) {
        ...
        if ((hp->h_len >= clicks) && (hp->h_len >= max_hp->h_len)) {
          max_hp = hp;
