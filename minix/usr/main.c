...
unsigned int first_inode_index_func(Diskfile* diskfile) {
    unsigned int i;
    unsigned int free_index;

    for (i=0; i<diskfile->block_nb; i++) {
        if (diskfile->inode_bitmap[i] == 0) {
            return i;
        }
    }
}
...
void add_file(Diskfile* diskfile, char *path) {
    ...
    unsigned int* free_blocks_index;
    unsigned int first_inode_index;
    ...
    /* Check if file fits */
    first_inode_index = first_inode_index_func(diskfile);
    free_blocks_index = free_block_indeces(diskfile);

    free_blocks_index[filename_in_blocks + file_in_blocks+1] = (unsigned int)-1;
    while(free_blocks_index[i] != (unsigned int)-1) {
        i++; 
    }
    if (i * DATA_SIZE< file_in_blocks + file_in_blocks) {
...
void remove_file(Diskfile* diskfile, char *filename) {
...
Inode* find_inode(Diskfile* diskfile, char* filename) {
    unsigned int i = (unsigned int)-1;
    unsigned int inode_index;
    Inode* curr_inode;
...
void extract_file(Diskfile* diskfile, char* filename) {
    ...
    output_file = fopen((const char*)filename, "wb");
